{if !$user->_logged_in}
	{* Supporto Tailwind: classi base e dark-mode tramite classe `dark` *}
	<body data-hash-tok="{$session_hash['token']}" data-hash-pos="{$session_hash['position']}" {if $system['theme_mode_night']}data-bs-theme="dark" {/if} class="tw font-sans antialiased {if $system['theme_mode_night']}night-mode dark{/if} visitor n_chat {if $page == 'index' && !$system['newsfeed_public']}index-body{/if}" {if $page == 'profile' && $system['system_profile_background_enabled'] && $profile['user_profile_background']}style="background: url({$profile['user_profile_background']}) fixed !important; background-size: 100% auto;" {/if}>
{else}
	{* Supporto Tailwind: classi base e dark-mode tramite classe `dark` *}
    <body data-hash-tok="{$session_hash['token']}" data-hash-pos="{$session_hash['position']}" data-chat-enabled="{$user->_data['user_chat_enabled']}" {if $system['theme_mode_night']}data-bs-theme="dark" {/if} class="tw font-sans antialiased {if $system['theme_mode_night']}night-mode dark{/if} {if !$system['chat_enabled'] || $user->_data['user_privacy_chat'] == "me"}n_chat{/if}{if $system['activation_enabled'] && !$system['activation_required'] && !$user->_data['user_activated']} n_activated{/if}{if !$system['system_live']} n_live{/if}" {if $page == 'profile' && $system['system_profile_background_enabled'] && $profile['user_profile_background']}style="background: url({$profile['user_profile_background']}) fixed !important; background-size: 100% auto;" {/if} {if $url}onload="initialize_scraper()" {/if}>
    {/if}
		<!-- main wrapper -->
														...
														<div class="dropdown-menu x_side_create_menu">
															...
															{if $user->_data['can_write_blogs']}
															  <a class="dropdown-item" href="{$system['system_url']}/blogs/new">
																...
																{__("Create Blog")} {if $system['paid_blogs_enabled']}<span class="badge rounded-pill text-bg-warning">{__("Paid")}</span>{/if}
															  </a>
															{/if}
															{if $system['market_enabled']}
															  <div class="dropdown-item pointer" data-toggle="modal" data-url="posts/product.php?do=create">
																...
																{__("Create Product")} {if $system['paid_products_enabled']}<span class="badge rounded-pill text-bg-warning">{__("Paid")}</span>{/if}
															  </div>
															{/if}
															{if $user->_data['can_raise_funding']}
															  <div class="dropdown-item pointer" data-toggle="modal" data-url="posts/funding.php?do=create">
																...
																{__("Create Funding")} {if $system['paid_funding_enabled']}<span class="badge rounded-pill text-bg-warning">{__("Paid")}</span>{/if}
															  </div>
															{/if}
