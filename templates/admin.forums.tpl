...
<div class="card">
  <div class="card-header with-icon">
    {if $sub_view == ""}
      <div class="float-end">
        <a href="{$system['system_url']}/{$control_panel['url']}/forums/add_forum" class="btn btn-md btn-primary">
          <i class="fa fa-plus mr5"></i>{__("Add New Forum")}
        </a>
      </div>
    {elseif $sub_view == "add_forum" || $sub_view == "edit_forum"}
      <div class="float-end">
        <a href="{$system['system_url']}/{$control_panel['url']}/forums" class="btn btn-md btn-light">
          <i class="fa fa-arrow-circle-left mr5"></i>{__("Go Back")}
        </a>
      </div>
    {elseif $sub_view == "find_threads"}
      <div class="float-end">
        <a href="{$system['system_url']}/{$control_panel['url']}/forums/threads" class="btn btn-md btn-light">
          <i class="fa fa-arrow-circle-left mr5"></i>{__("Go Back")}
        </a>
      </div>
    {elseif $sub_view == "find_replies"}
      <div class="float-end">
        <a href="{$system['system_url']}/{$control_panel['url']}/forums/replies" class="btn btn-md btn-light">
          <i class="fa fa-arrow-circle-left mr5"></i>{__("Go Back")}
        </a>
      </div>
    {/if}
  ...
  {elseif $sub_view == "threads" || $sub_view == "find_threads"}
    ...
    <div class="card-body">
      ...
      <!-- search form -->
      <div class="mb20">
        <form class="d-flex flex-row align-items-center flex-wrap" action="{$system['system_url']}/{$control_panel['url']}/forums/find_threads" method="get">
            ...
            {if $rows}
              {foreach $rows as $row}
                <tr>
                  ...
                  <td>
                    <button data-bs-toggle="tooltip" title='{__("Delete")}' class="btn btn-sm btn-icon btn-rounded btn-danger js_admin-deleter" data-handle="forum_thread" data-id="{$row['thread_id']}">
  ...
  {elseif $sub_view == "replies" || $sub_view == "find_replies"}
    ...
    <div class="card-body">
      ...
      <!-- search form -->
      <div class="mb20">
        <form class="d-flex flex-row align-items-center flex-wrap" action="{$system['system_url']}/{$control_panel['url']}/forums/find_replies" method="get">
          <div class="form-group mb0">
            <div class="input-group">
              <input type="text" class="form-control" name="query">
              <button type="submit" class="btn btn-sm btn-light"><i class="fas fa-search mr5"></i>{__("Search")}</button>
            ...
            {if $rows}
              {foreach $rows as $row}
                <tr>
                  ...
                  <td>
                    <button data-bs-toggle="tooltip" title='{__("Delete")}' class="btn btn-sm btn-icon btn-rounded btn-danger js_admin-deleter" data-handle="forum_reply" data-id="{$row['reply_id']}">
                      <i class="fa fa-trash-alt"></i>
                    </button>
                  </td>
                </tr>
