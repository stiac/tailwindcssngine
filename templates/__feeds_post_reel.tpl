<!-- post body -->
<!-- Tailwind: spacing and base typography for reel post layout -->
<div class="post-body px-3 pb-3 pt-2 text-slate-900">
	<!-- post header -->
	<!-- Tailwind: flex alignment and spacing for avatar + meta -->
	<div class="d-flex x_user_info post-header flex items-start gap-3">
		<!-- post picture -->
		...
		<div class="post-avatar position-relative flex-0 inline-flex items-center justify-center">
			{if $post['is_anonymous']}
				<!-- Tailwind: anonymous avatar styling -->
			...
			{else}
				<!-- Tailwind: avatar ring + hover transition -->
			...
			<div class="post-meta space-y-1">
				...
				{else}
					...
					{if $post['user_type'] == "user"}
						<span class="text-muted small text-slate-500">@{$post['user_name']}</span>
					{/if}
					{if $post['user_type'] == "page"}
						<span class="text-muted small text-slate-500">@{$post['page_name']}</span>
					{/if}
				{/if}
				<!-- post author -->

				<!-- post time & location & privacy -->
				<!-- Tailwind: muted metadata text and spacing -->
	...
	<div class="post-stats d-flex align-items-center mt-2 pt-1 text-sm text-slate-500 flex flex-wrap items-center gap-2">
		<!-- reactions stats -->
	...
	<!-- post actions -->
	{if $user->_logged_in}
		<!-- Tailwind: action button layout and spacing -->
		<div class="post-actions d-flex align-items-center justify-content-between gap-2 flex flex-wrap">
			<!-- reactions -->
			<!-- Tailwind: action button visuals -->
			<div class="action-btn position-relative rounded-circle main_bg_half p-2 lh-1 pointer unselectable reactions-wrapper bg-slate-100/70 hover:bg-slate-200/70 transition {if $post['i_react']}js_unreact-post{/if}" data-reaction="{$post['i_reaction']}">
				<!-- reaction-btn -->
			...
			<!-- comment -->
			<div class="action-btn position-relative rounded-circle main_bg_half p-2 lh-1 pointer js_comment bg-slate-100/70 hover:bg-slate-200/70 transition {if $post['comments_disabled']}x-hidden{/if}">
			...
			{if $post['privacy'] == "public"}
				<div class="action-btn position-relative rounded-circle main_bg_half p-2 lh-1 pointer bg-slate-100/70 hover:bg-slate-200/70 transition" data-toggle="modal" data-url="posts/share.php?do=create&post_id={$post['post_id']}">
		...
		{if $post['author_id'] != $user->_data['user_id'] && $post['tips_enabled']}
			<!-- tips -->
			<!-- Tailwind: tip button spacing and hover feedback -->
			<div class="post-tips flex flex-wrap gap-2">
				<button class="btn btn-sm btn-primary transition hover:bg-blue-600" data-toggle="modal" data-url="#send-tip" data-options='{ "id": "{$post['author_id']}", "value": "1"}'>
				  {print_money(1)}
				</button>
				<button class="btn btn-sm btn-primary transition hover:bg-blue-600" data-toggle="modal" data-url="#send-tip" data-options='{ "id": "{$post['author_id']}", "value": "5"}'>
				  {print_money(5)}
				</button>
				<button class="btn btn-sm btn-primary transition hover:bg-blue-600" data-toggle="modal" data-url="#send-tip" data-options='{ "id": "{$post['author_id']}", "value": "10"}'>
				  {print_money(10)}
				</button>
				<button class="btn btn-sm btn-primary transition hover:bg-blue-600" data-toggle="modal" data-url="#send-tip" data-options='{ "id": "{$post['author_id']}", "value": "20"}'>
				  {print_money(20)}
				</button>
				<button class="btn btn-sm btn-primary transition hover:bg-blue-600" data-toggle="modal" data-url="#send-tip" data-options='{ "id": "{$post['author_id']}", "value": "50"}'>
				  {print_money(50)}
				</button>
				<button class="btn btn-sm btn-primary transition hover:bg-blue-600" data-toggle="modal" data-url="#send-tip" data-options='{ "id": "{$post['author_id']}", "value": "100"}'>
				  {print_money(100)}
				</button>
				<button class="btn btn-sm btn-primary transition hover:bg-blue-600" data-toggle="modal" data-url="#send-tip" data-options='{ "id": "{$post['author_id']}"}'>
				  $$$
				</button>
			</div>
			<!-- tips -->
