{if !$standalone}<li>{/if}
	...
	{if $post['source'] == "popular"}
		<div class="fw-medium small p-3 pb-0 d-flex align-items-center gap-1 text-muted text-xs font-medium text-slate-500 flex items-center gap-2 px-4 pt-4 pb-2">
	...
	<!-- Tailwind: card wrapper utilities for spacing, borders, and shadow -->
	<div class="bg-white position-relative p-3 post rounded-xl border border-slate-200/70 shadow-sm ring-1 ring-slate-100/70 
		...
		{if ($post['is_pending']) OR ($post['in_group'] && !$post['group_approved']) OR ($post['in_event'] && !$post['event_approved'])}
			<div class="text-secondary fw-bold small mb-2 d-flex align-items-center gap-1 text-xs font-semibold text-slate-500 flex items-center gap-2">
		...
		{if $standalone && $pinned}
			<div class="text-warning fw-bold small mb-2 d-flex align-items-center gap-1 text-xs font-semibold text-amber-500 flex items-center gap-2">
		...
		{if $standalone && $boosted}
			<div class="text-orange fw-bold small mb-2 d-flex align-items-center gap-1 text-xs font-semibold text-orange-500 flex items-center gap-2">
		...
		{if $_get == "posts_profile" && $user->_data['user_id'] == $post['author_id'] && ($post['is_hidden'] || $post['is_anonymous'])}
			<div class="post-top-alert small text-white text-center fw-semibold p-2 text-xs font-semibold bg-slate-900/90 rounded-md">{__("Only you can see this post")}</div>
		{/if}
		<!-- post top alert -->

		<!-- memory post -->
		{if $_get == "memories"}
			<div class="post-memory-header main fw-semibold mb-3 text-sm font-semibold text-slate-700 border-b border-slate-200 pb-2">
		...
		<!-- post body -->
		<div class="post-body text-slate-900">
					...
					<div class="flex-1">
						...
						<div class="post-stats d-flex align-items-center text-sm text-slate-500 flex flex-wrap items-center gap-2">
							...
							{if $post['post_type'] == "funding"}
								<span class="pointer text-slate-500 hover:text-slate-700 transition" data-toggle="modal" data-url="posts/who_donates.php?post_id={$post['post_id']}">
							...
							{if $system['posts_reviews_enabled']}
								<!-- reviews -->
								<span class="pointer text-slate-500 hover:text-slate-700 transition" data-toggle="modal" data-url="posts/who_reviews.php?post_id={$post['post_id']}">
						...
						{if $user->_logged_in && $_get != "posts_information"}
							<div class="post-actions d-flex align-items-center justify-content-between mt-3 border-t border-slate-200/80 pt-3">
								<!-- reactions -->
