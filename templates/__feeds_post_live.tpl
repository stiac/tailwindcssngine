<!-- post body -->
<!-- Tailwind: spacing, typography, and neutral background for live post layout -->
<div class="post-body pt0 text-slate-900">
	<!-- post header -->
	<!-- Tailwind: flex alignment and gaps for avatar + meta -->
	<div class="d-flex position-relative x_user_info post-header flex items-start gap-3">
		<!-- post picture -->
		<!-- Tailwind: avatar wrapper sizing and alignment -->
		<div class="post-avatar position-relative flex-0 inline-flex items-center justify-center">
			{if $post['is_anonymous']}
				<!-- Tailwind: anonymous avatar layout -->
				<div class="post-avatar-anonymous text-white rounded-circle overflow-hidden d-flex align-items-center justify-content-center bg-slate-800/90 shadow-sm">
			...
			{else}
				<!-- Tailwind: avatar border ring + hover transition -->
				<a class="post-avatar-picture rounded-circle overflow-hidden d-block ring-1 ring-slate-200/70 shadow-sm transition" href="{$post['post_author_url']}" style="background-image:url({$post['post_author_picture']});"></a>
				{if $post['post_author_online']}<span class="online-dot position-absolute rounded-circle"></span>{/if}
			{/if}
		</div>
		<!-- post picture -->
		
		<!-- Tailwind: meta spacing and width control -->
		<div class="mw-0 mx-2 flex-1">
			<!-- post meta -->
			<div class="post-meta space-y-1">
				<!-- post author -->
				{if $post['is_anonymous']}
					<span class="post-author fw-semibold text-slate-700">{__("Anonymous")}</span>
				{else}
					<span class="js_user-popover" data-type="{$post['user_type']}" data-uid="{$post['user_id']}">
						<a class="post-author fw-semibold body-color text-slate-900 hover:text-slate-700 transition" href="{$post['post_author_url']}">{$post['post_author_name']}</a>
					</span>
				...
				<!-- Tailwind: small muted text and spacing for metadata -->
				<div class="post-time text-muted text-xs text-slate-500 flex flex-wrap items-center gap-1">
					<a href="{$system['system_url']}/posts/{$post['post_id']}" class="js_moment text-muted text-slate-500 hover:text-slate-700 transition" data-time="{$post['time']}">{$post['time']}</a>
					{if $post['location']}
	...
	<!-- Tailwind: spacing and alignment for stats/actions row -->
	<div class="d-flex mt-2 pt-1 flex items-start gap-3">
		<div class="post_empty_space flex-0"></div>
		<div class="flex-1 space-y-2">
			<!-- post stats -->
			<!-- Tailwind: stats typography and spacing -->
			<div class="post-stats d-flex align-items-center text-sm text-slate-500 flex flex-wrap items-center gap-2">
				<!-- reactions stats -->
					...
					<div class="pointer" data-toggle="modal" data-url="posts/who_reacts.php?post_id={$post['post_id']}">
						<div class="reactions-stats d-flex align-items-center flex items-center gap-1">
							{foreach $post['reactions'] as $reaction_type => $reaction_count}
								{if $reaction_count > 0}
									<div class="reactions-stats-item bg-white rounded-circle position-relative d-inline-flex align-middle align-items-center justify-content-center border border-slate-200/70 shadow-sm">
				...
				<!-- comments -->
				<span class="pointer js_comments-toggle text-slate-500 hover:text-slate-700 transition">
					{$post['comments_formatted']} {__("Comments")}
				</span>
				<!-- comments -->

				<!-- shares -->
				<span class="pointer {if $post['shares'] == 0}x-hidden{/if} text-slate-500 hover:text-slate-700 transition" data-toggle="modal" data-url="posts/who_shares.php?post_id={$post['post_id']}">
					{$post['shares_formatted']} {__("Shares")}
				</span>
				<!-- shares -->
			</div>
			<!-- post stats -->
			...
			<!-- post actions -->
			{if $user->_logged_in}
				<!-- Tailwind: button layout and spacing -->
				<div class="post-actions d-flex align-items-center justify-content-between mx-0 gap-2 flex flex-wrap">
					<!-- reactions -->
					<div class="action-btn position-relative rounded-circle main_bg_half p-2 lh-1 pointer unselectable reactions-wrapper inline-flex items-center justify-center rounded-full bg-slate-100/80 text-slate-600 hover:bg-slate-200/80 transition {if $post['i_react']}js_unreact-post{/if}" data-reaction="{$post['i_reaction']}">
						<!-- reaction-btn -->
					...
					<!-- comment -->
					<div class="action-btn position-relative rounded-circle main_bg_half p-2 lh-1 pointer js_comment inline-flex items-center justify-center rounded-full bg-slate-100/80 text-slate-600 hover:bg-slate-200/80 transition {if $post['comments_disabled']}x-hidden{/if}">
					...
					{if $post['privacy'] == "public"}
						<div class="action-btn position-relative rounded-circle main_bg_half p-2 lh-1 pointer inline-flex items-center justify-center rounded-full bg-slate-100/80 text-slate-600 hover:bg-slate-200/80 transition" data-toggle="modal" data-url="posts/share.php?do=create&post_id={$post['post_id']}">
				...
				{if $post['author_id'] != $user->_data['user_id'] && $post['tips_enabled']}
					<!-- tips -->
					<!-- Tailwind: tip buttons group -->
					<div class="post-tips flex flex-wrap gap-2">
						<button class="btn btn-sm btn-primary inline-flex items-center justify-center rounded-full bg-slate-900 text-white hover:bg-slate-800 transition" data-toggle="modal" data-url="#send-tip" data-options='{ "id": "{$post['author_id']}", "value": "1"}'>
						  {print_money(1)}
						</button>
						<button class="btn btn-sm btn-primary inline-flex items-center justify-center rounded-full bg-slate-900 text-white hover:bg-slate-800 transition" data-toggle="modal" data-url="#send-tip" data-options='{ "id": "{$post['author_id']}", "value": "5"}'>
						  {print_money(5)}
						</button>
						<button class="btn btn-sm btn-primary inline-flex items-center justify-center rounded-full bg-slate-900 text-white hover:bg-slate-800 transition" data-toggle="modal" data-url="#send-tip" data-options='{ "id": "{$post['author_id']}", "value": "10"}'>
						  {print_money(10)}
						</button>
						<button class="btn btn-sm btn-primary inline-flex items-center justify-center rounded-full bg-slate-900 text-white hover:bg-slate-800 transition" data-toggle="modal" data-url="#send-tip" data-options='{ "id": "{$post['author_id']}", "value": "20"}'>
						  {print_money(20)}
						</button>
						<button class="btn btn-sm btn-primary inline-flex items-center justify-center rounded-full bg-slate-900 text-white hover:bg-slate-800 transition" data-toggle="modal" data-url="#send-tip" data-options='{ "id": "{$post['author_id']}", "value": "50"}'>
						  {print_money(50)}
						</button>
						<button class="btn btn-sm btn-primary inline-flex items-center justify-center rounded-full bg-slate-900 text-white hover:bg-slate-800 transition" data-toggle="modal" data-url="#send-tip" data-options='{ "id": "{$post['author_id']}", "value": "100"}'>
						  {print_money(100)}
						</button>
						<button class="btn btn-sm btn-primary inline-flex items-center justify-center rounded-full bg-slate-900 text-white hover:bg-slate-800 transition" data-toggle="modal" data-url="#send-tip" data-options='{ "id": "{$post['author_id']}"}'>
						  $$$
						</button>
					</div>
					<!-- tips -->
