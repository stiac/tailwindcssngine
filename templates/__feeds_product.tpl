{* Product card wrapper with Tailwind utilities for spacing and layout *}
<div class="col-6 col-xl-3 pb-2 mb-4 w-full sm:w-1/2 xl:w-1/4">
	<div class="position-relative product {if $_boosted}boosted{/if} relative">
		{if $_boosted}
			{* Boosted badge *}
			<div class="position-absolute m-2 rounded-2 fw-medium small text-white boosted-icon absolute left-0 top-0 inline-flex items-center rounded-full bg-amber-500/90 px-2 py-0.5 text-xs font-semibold uppercase tracking-wide text-white shadow-sm">
				{__("Promoted")}
			</div>
		{/if}
		...
		{else}
			{* Product image *}
			<a href="{$system['system_url']}/posts/{$post['post_id']}" class="product-image d-block position-relative w-100 overflow-hidden relative block w-full overflow-hidden rounded-2xl bg-slate-200 shadow-sm transition hover:shadow-md">
				{if $post['photos_num'] > 0}
					<img src="{$system['system_uploads']}/{$post['photos'][0]['source']}" class="w-100 h-100 object-cover">
				{else}
					<img src="{$system['system_url']}/content/themes/{$system['theme']}/images/blank_product.png" class="w-100 h-100 object-cover">
				{/if}
				{* Stock status overlay *}
				<div class="d-flex align-items-end gap-2 small text-white position-absolute p-2 stock_stat absolute inset-x-0 bottom-0 flex items-end gap-2 p-3 text-xs text-white">
					<div class="d-flex align-items-center gap-2 flex flex-wrap items-center gap-2">
						...
						<div class="d-flex flex flex-wrap items-center gap-2">
			...
			<div class="mt-2 space-y-1">
				<h6 class="title text-truncate {if $system['posts_reviews_enabled']}mb-0{/if} text-sm font-semibold text-slate-900"><a href="{$system['system_url']}/posts/{$post['post_id']}" class="body-color text-slate-900 hover:text-slate-950">{$post['product']['name']}</a></h6>
				
				{if $system['posts_reviews_enabled']}
					<div class="d-inline-flex align-items-center gap-2 small mb-2 pointer inline-flex items-center gap-2 text-xs text-slate-600" data-toggle="modal" data-url="posts/who_reviews.php?post_id={$post['post_id']}">
						<span class="review-stars small inline-flex items-center gap-1">
				...
				<h6 class="main text-truncate text-base font-semibold text-slate-900">
				...
				<div class="d-flex align-items-center gap-2 small flex items-center gap-2 text-xs text-slate-600">
