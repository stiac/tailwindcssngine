<div class="{if $_small}w-1/3{else}w-1/2 md:w-1/3 lg:w-1/6{/if} {if $photo['blur']}x-blured{/if}">
	<!-- Photo card container using Tailwind utilities for layout, spacing, and visuals -->
	<a class="pg_photo relative block h-full w-full rounded-lg bg-cover bg-center shadow-sm transition hover:shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-sky-500 {if !$_small}large{/if} js_lightbox" href="{$system['system_url']}/photos/{$photo['photo_id']}" data-id="{$photo['photo_id']}" data-image="{$system['system_uploads']}/{$photo['source']}" data-context="{$_context}" style='background-image:url("{$system['system_uploads']}/{$photo['source']}");'>
		{if !$_small && ($_manage || $photo['manage'])}
			<!-- Delete action button styled with Tailwind utilities -->
			<div class="pg_photo-delete-btn absolute right-2 top-2">
				<button type="button" class="js_delete-photo inline-flex h-8 w-8 items-center justify-center rounded-full bg-white/90 text-slate-700 shadow-sm ring-1 ring-slate-200 transition hover:bg-white focus:outline-none focus-visible:ring-2 focus-visible:ring-sky-500" data-id="{$photo['photo_id']}" data-bs-toggle="tooltip" title='{__("Delete")}'>
