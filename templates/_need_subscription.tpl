...
<div class="{if $subscriptions_image}pg_wrapper overflow-hidden clearfix mt-2 position-relative need-subscription{/if}">
	...
	<div class="p-3 {if $subscriptions_image}position-absolute w-100 h-100 bg-black bg-opacity-50 need-subscription-innr d-flex align-items-center justify-content-center{/if}">
		<div class="{if $subscriptions_image}text-white{else}text-muted{/if} text-center py-5">
			...
			{if isset($price)}
				<div class="mt-3">
					<button class="btn btn-main" data-toggle="modal" data-url="monetization/controller.php?do=get_plans&node_id={$node_id}&node_type={$node_type}" data-size="large">
