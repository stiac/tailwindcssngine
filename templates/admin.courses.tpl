...
<div class="card">
  <div class="card-header with-icon">
    {if $sub_view == "find"}
      <div class="float-end">
        <a href="{$system['system_url']}/{$control_panel['url']}/courses" class="btn btn-md btn-light">
          <i class="fa fa-arrow-circle-left mr5"></i>{__("Go Back")}
        </a>
      </div>
    {elseif $sub_view == "categories"}
      <div class="float-end">
        <a href="{$system['system_url']}/{$control_panel['url']}/courses/add_category" class="btn btn-md btn-primary">
          <i class="fa fa-plus mr5"></i>{__("Add New Category")}
        </a>
      </div>
    {elseif $sub_view == "add_category" || $sub_view == "edit_category"}
      <div class="float-end">
        <a href="{$system['system_url']}/{$control_panel['url']}/courses/categories" class="btn btn-md btn-light">
          <i class="fa fa-arrow-circle-left mr5"></i>{__("Go Back")}
        </a>
      </div>
    {/if}
    <i class="fa fa-book mr10"></i>{__("Courses")}
  ...
  {if $sub_view == "" || $sub_view == "find"}
    ...
    <div class="card-body">
      ...
      <!-- search form -->
      <div class="mb20">
        <form class="d-flex flex-row align-items-center flex-wrap" action="{$system['system_url']}/{$control_panel['url']}/courses/find" method="get">
        ...
        <div class="form-text small">
          {__("Search by Course Title or Description")}
        </div>
      </div>
      <!-- search form -->
            ...
            {if $rows}
              {foreach $rows as $row}
                <tr>
                  <td>{$row['post_id']}</td>
                  <td>
                    <a target="_blank" href="{$row['course_author_url']}">
                      <img class="tbl-image" src="{$row['course_author_picture']}">
                      {$row['course_author_name']}
                    </a>
                  </td>
                  <td>
      ...
      <div class="table-responsive">
        <table class="table table-striped table-bordered table-hover js_treegrid">
          ...
          <tbody>
            {if $rows}
              {foreach $rows as $row}
                {include file='__categories.recursive_rows.tpl' _url="courses" _handle="courses_category"}
              {/foreach}
            {else}
              <tr>
  ...
  {elseif $sub_view == "add_category"}
    ...
    <form class="js_ajax-forms" data-url="admin/courses.php?do=add_category">
      <div class="card-body">
  ...
  {elseif $sub_view == "edit_category"}
    ...
    <form class="js_ajax-forms" data-url="admin/courses.php?do=edit_category&id={$data['category_id']}">
      <div class="card-body">
        <div class="row form-group">
          <label class="col-md-3 form-label">
            {__("Name")}
