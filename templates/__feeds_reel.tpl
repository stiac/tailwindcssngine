<div class="reel-container position-absolute absolute inset-0 transition-all duration-200{if $_hidden || $_iteration > 1} hidden{/if}" data-id="{$post['post_id']}">
	<!-- Contenitore principale del reel con supporto Tailwind CSS per posizione e transizioni -->
	<div class="position-relative relative w-100 h-100 w-full h-full">
		<div class="reel-video-wrapper position-relative relative w-100 h-100 w-full h-full select-none">
			<!-- Navigazione reel: pulsante precedente -->
			<div class="btn btn-gray reel-prev-btn d-none d-lg-flex rounded-circle p-2 position-absolute absolute pointer js_reel-prev-btn items-center justify-center">
			...
			<!-- Navigazione reel: pulsante successivo -->
			<div class="btn btn-gray reel-next-btn d-none d-lg-flex rounded-circle p-2 position-absolute absolute pointer js_reel-next-btn items-center justify-center">
			...
			<div class="reel-video-container position-relative relative w-100 h-100 w-full h-full">
				<!-- Player video con classi Tailwind per dimensioni full -->
				<video class="w-100 h-100 w-full h-full js_video-plyr" data-reel="true" id="reel-{$post['reel']['reel_id']}" {if $user->_logged_in}onplay="update_media_views('reel', {$post['reel']['reel_id']})" {/if} {if $post['reel']['thumbnail']}data-poster="{$system['system_uploads']}/{$post['reel']['thumbnail']}" {/if} preload="auto" {if $_iteration == 1}autoplay{/if} playsinline preload="auto">
				...
				<!-- Overlay descrizione e controlli con supporto Tailwind -->
				<div class="position-absolute absolute inset-x-0 bottom-0 d-flex flex items-end py-3 video-caption-overlay">
					<div class="video-caption px-3 text-white flex-1">
						{if $post['text']}
							{include file='__feeds_post.text.tpl'}
						{/if}
					</div>
					<div class="video-controlls flex-0 flex-shrink-0">
						<div class="reel-actions d-flex flex flex-column text-center gap-2 gap-md-3 space-y-2 md:space-y-3">
							<!-- reactions -->
							<div class="reel-action-btn">
								<div class="btn btn-gray rounded-circle p-2 lh-1 action-btn position-relative relative pointer unselectable reactions-wrapper {if $post['i_react']}js_unreact-post{/if}" data-reaction="{$post['i_reaction']}">
									<!-- reaction-btn -->
									<div class="reaction-btn position-relative relative m-1">
										{if !$post['i_react']}
									...
									<!-- reactions-container -->
									<div class="reactions-container position-absolute absolute">
								...
								<!-- reactions stats -->
								<span class="pointer d-block mt-1 small fw-medium text-xs font-medium" data-toggle="modal" data-url="posts/who_reacts.php?post_id={$post['post_id']}">
							...
							<!-- comment -->
							<div class="reel-action-btn">
								<div class="btn btn-gray rounded-circle p-2 lh-1 action-btn position-relative relative pointer js_reel-comments-toggle">
								...
								<span class="pointer d-block mt-1 small fw-medium text-xs font-medium js_reel-comments-toggle">{$post['comments_formatted']}</span>
							</div>
							<!-- comment -->
							...
							{if $post['privacy'] == "public" || ($post['in_group'] && $post['group_privacy'] == "public") || ($post['in_event'] && $post['event_privacy'] == "public") }
								<div class="reel-action-btn">
									<div class="btn btn-gray rounded-circle p-2 lh-1 action-btn position-relative relative pointer" data-toggle="modal" data-url="posts/share.php?do=create&post_id={$post['post_id']}">
									...
									<!-- shares -->
									<span class="pointer d-block mt-1 small fw-medium text-xs font-medium" data-toggle="modal" data-url="posts/who_shares.php?post_id={$post['post_id']}">
							...
							<!-- post picture -->
							<div class="post-avatar position-relative relative">
								{if $post['is_anonymous']}
									<div class="post-avatar-anonymous text-white rounded-circle overflow-hidden d-flex flex items-center justify-center">
		...
		<div class="reel-comments-wrapper position-absolute absolute content w-100 w-full">
			<!-- Pannello commenti con controlli Tailwind per spaziatura -->
			<div class="d-flex flex justify-content-end px-2 pt-2">
